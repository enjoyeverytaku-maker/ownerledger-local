...
ipcMain.handle("reports:exportTaxCsv", async (_event, targetYear) => {
  ...
  const [charges, expenses, deposits, documents] = await Promise.all([
    ...
    prisma.expense.findMany({ where: { spentAt: { gte: yearStart, lt: yearEnd }, archivedAt: null, status: { not: "取消" } }, include: { property: true, unit: true } }),
    ...
    prisma.document.findMany({ where: { archivedAt: null } })
  ]);
  const rows = [
    ...charges.map((item) => ({
      種類: "年間収入",
  ...
  const csv = "\uFEFF" + toCsv(["種類", "日付", "物件", "部屋", "相手先", "区分", "金額", "メモ"], rows);
